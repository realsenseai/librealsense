        ...
        if check.is_true( f, msg='no frame received' ):
            ...
            if check.is_true( odf, msg='frame is not an object_detection_frame' ):
                ...
                if check.equal( count, 2 ):
                    ...
                    det0_3d = odf.get_detection_3d( 0 )
                    check.equal( det0_3d.com_valid, 0 )
                    check.equal( det0_3d.world_position.z, 0.0 )
        ...
        if check.is_true( f, msg='no frame received' ):
            ...
            if check.is_true( odf, msg='frame is not an object_detection_frame' ):
                ...
                if check.equal( odf.get_detection_count(), 1 ):
                    ...
                    det3d = odf.get_detection_3d( 0 )
                    check.equal( det3d.com_valid, 1 )
                    check.is_true( abs( det3d.detection.depth - 2.291 ) < 1e-5 )
                    check.is_true( abs( det3d.world_position.x - 1.0 ) < 1e-5 )
                    check.is_true( abs( det3d.world_position.y + 0.5 ) < 1e-5 )
                    check.is_true( abs( det3d.world_position.z - 2.0 ) < 1e-5 )
                    check.is_true( abs( det3d.image_x - 321.5 ) < 1e-5 )
                    check.is_true( abs( det3d.image_y - 181.25 ) < 1e-5 )
