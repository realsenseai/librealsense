...
def find_any_two_devices_or_exit():
    ...
    This is useful for tests that can work with any combination of devices (e.g., D400 + L500).
    If fewer than two devices are available, the test will be skipped (not failed) with a clear message.
    ...
    if devices_list.size() < 2:
        log.f( f"Test requires 2 devices; found {devices_list.size()} device(s)" )
...
class two_devices:
    ...
    Usage examples:
        ...
        with test.two_devices() as (dev1, dev2):
            # dev1 and dev2 may be different product lines (e.g., D400 + L500)
    ...
    def __init__(self, product_line=None):
        """
        :param product_line: Optional. The product line of devices to search for (e.g., rs.product_line.D400).
    ...
    def __enter__(self):
        ...
        if self.product_line is not None:
            ...
            self.dev1, self.dev2, self.context = find_any_two_devices_or_exit()
