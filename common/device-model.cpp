...
namespace rs2
{
        ...
        for (auto&& sub : subdevices)
        {
            ...
            if (ImGui::TreeNodeEx(label.c_str(), flags))
            {
                ...
                if (sub->s->is<depth_sensor>()) {
                    ...
                    {
                        control_section effect(pb->get_name(), pb->get_name() + "##" + id);
        ...
        control_draw_context& ctx)
    {
        ...
        for (auto&& pb : sub->post_processing)
        {
            auto & filter = section.add_section(pb->get_name(), pb->get_name() + "##" + id);
        ...
        control_draw_context& ctx)
    {
        ...
        for (auto&& pb : sub->embedded_filters)
        {
            ...
            filter.toggle = [this, sub, pb, &window, windows_width, err](ImVec2 pos)
            {
                ImGui::SetCursorPos({ windows_width - 42, pos.y - 3 });
